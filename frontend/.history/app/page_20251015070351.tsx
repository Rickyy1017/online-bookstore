...
export default function Page() {
  ...
  const handleInfoSubmit = async () => {
    ...
    if (!phone) {
      ...
      setPhoneError('Please enter a valid phone number (e.g., 09138246303).');
      valid = false;
    }

    if (!valid) return;

    const message = `New contact: Email: ${email}, Phone: ${phone}`;

    // Send email via mailto
    const emailLink = `mailto:ericjo6303@gmail.com?subject=New Contact&body=${encodeURIComponent(message)}`;
    window.location.href = emailLink;

    // Send WhatsApp message
    const whatsappLink = `https://wa.me/2349138246303?text=${encodeURIComponent(message)}`;
    window.open(whatsappLink, '_blank');

    alert('Information submitted successfully! Email client opened and WhatsApp message sent.');
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter your phone number (e.g., 09138246303)"
                />
