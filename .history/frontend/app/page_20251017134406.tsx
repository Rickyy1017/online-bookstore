...
export default function Page() {
  ...
  const checkout = () => {
    toast('Checkout functionality not implemented yet.', { icon: 'ℹ️' });
  };
  ...
  const handleInfoSubmit = async () => {
    ...
    if (!phone) {
      ...
      setPhoneError('Please enter a valid phone number (e.g., 09123456789).');
      valid = false;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        // persist submission for this browser session only
      ...
      toast.error('Error submitting information. Please check if the backend is running.');
    }
  ...
  useEffect(() => {
    // prefer sessionStorage so modal appears again in a new browser session
  ...
  // developer / user utility to clear the submitted flag and reopen modal
  const resetModal = () => {
    try {
    ...
    setShowModal(true);
    toast('Modal reset for this session', { icon: '🔄' });
  };
      ...
      {!hasSubmitted && showModal && (
        <div className="fixed inset-0 backdrop-blur-md flex justify-center items-center z-50">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-md w-full mx-4">
            ...
            <p className="text-md text-red-600 mb-4">Please fill to proceed to page</p>
