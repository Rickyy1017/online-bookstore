...
export default function SiteHeader() {
  ...
  return (
    <header className="relative z-10 bg-white shadow-md border-b">
      ...
      {/* Backdrop (only rendered when open to avoid layout effects) */}
      {mobileNavOpen && (
        <div
      ...
      {/* Panel (fixed overlay so it doesn't push content) */}
      <div
        className={`md:hidden fixed inset-x-0 top-0 z-50 transform transition-transform transition-opacity duration-300 ease-in-out ${mobileNavOpen ? 'translate-y-0 opacity-100 pointer-events-auto' : '-translate-y-full opacity-0 pointer-events-none'}`}
        aria-hidden={!mobileNavOpen}
      >
        {/* Keep the visual menu below the header by adding top padding inside the panel */}
